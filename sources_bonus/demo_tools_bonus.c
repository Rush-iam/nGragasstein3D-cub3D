...
{
	static short	shift;
	const t_point	p1 = {(int)mlx->img.size.x / 2, (int)mlx->img.size.y / 2};
	t_point			p2;
	int				i;
	...
	while (i++ < rays)
	{
		p2.x = p1.x + mlx->img.size.y * cosf(((shift + i) * 2 * M_PI_F) / 360);
		p2.y = p1.y + mlx->img.size.y * sinf(((shift + i) * 2 * M_PI_F) / 360);
